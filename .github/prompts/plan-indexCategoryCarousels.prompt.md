...
1. **Add configuration in `app/config.py`**: Add `INDEX_CAROUSEL_COUNT` (default 4), `INDEX_CAROUSEL_ITEMS_PER_CATEGORY` (default 10), and `INDEX_CAROUSEL_CATEGORIES` (default None, list of category IDs when explicitly set).

2. **Create utility function in `app/routes/utils.py`**: Add `get_index_carousel_categories()` returning up to N categories with listings—uses explicit category IDs from config if set, otherwise queries top 2N by listing count and randomly selects N; filters out empty categories.

3. **Modify `app/routes/listings.py` `index()` function**: Call `get_index_carousel_categories()`; for each category, fetch up to M most recent listings; check if any categories exist (controls "+ Post New Listing" button); pass `category_carousels` to template; remove pagination logic.

4. **Update `app/templates/index.html`**: Replace grid and pagination with conditional logic for empty states (no categories → admin message; categories exist but no listings → no listings message); loop rendering Bootstrap carousels with unique `id`, clickable category heading using `url_for('listings.category_filtered_listings', category_path=category.url_path)`, reused card HTML (card-dimensions, image, title, price), and prev/next controls.

5. **Add carousel CSS in `app/static/css/styles.css`**: Add spacing between carousels; use Bootstrap's default responsive carousel behavior for card layout; ensure listing cards maintain 192px × 256px dimensions; position carousel controls.
