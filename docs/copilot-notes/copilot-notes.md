...
- [✓] Defined all primary database models (`User`, `Type`, `Category`, `Item`) in `models.py` with proper relationships and constraints.
- [✓] Set up `app.py` as the Flask application entry, using the application factory pattern and correct import order to avoid circular dependencies.
- [✓] Registered blueprints for main routes under `routes/` (authentication, items, users, admin).
- [✓] Integrated Flask-Login with a custom user loader and login view.
- [✓] Added Flask-Mail setup for email notifications.
- [✓] Implemented CLI command (`init-db`) for database initialization and creation of default admin/types/categories.
- [✓] Used Flask's `before_request` to inject sidebar/type data into templates.
- [✓] Implemented authentication and registration routes (login, logout, registration, forgot/reset password, LDAP support) in `routes/auth.py`.
- [✓] Implemented item listing, creation, editing, and deletion in `routes/items.py`, with corresponding templates.
- [✓] Implemented admin management views for users (list, edit, delete), types (full CRUD), and categories (full CRUD). Admins can also delete any item.
- [✓] Deployment scripts and instructions are complete. The repo includes a Dockerfile and docker-compose.yml for containerized deployment, with detailed usage instructions and environment variable documentation in the README. No systemd or NGINX configuration is provided at this stage, but can be added if needed later.
...
## TODO Fun List :-)
- [✓] Investigate backend and frontend mapping of first/last name columns
- [✓] Understand why "Edit" is a link but "Delete" is a form button
- [✓] Compare the best approach: vanilla JS vs jQuery for UI scripting
- [✓] Implement "Select All" with jQuery in admin_items.html
- [✓] Sync "Select All" checkbox state with row selections
- [✓] Debug and fix checkbox syncing logic
- [✓] Celebrate a perfect working solution! 🎉
