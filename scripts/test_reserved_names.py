...
def run_tests():
    ...
    with app.app_context():
        db.create_all()
        # Seed admin user
        admin = User(email="admin@classifieds.io", first_name="Admin", last_name="User", is_admin=True)
        ...
        # Login as admin
        resp = client.post('/auth/login', data={'email': 'admin@classifieds.io', 'password': 'admin'}, follow_redirects=True)
        results['login_status'] = resp.status_code
        results['login_contains'] = b"Logged in successfully" in resp.data

        # 1) Reserved name: 'admin'
        resp = client.post('/admin/categories/new', data={'name': 'admin', 'parent_id': '0', 'submit': 'Save'}, follow_redirects=True)
        ...
        # 2) Empty-slug: '_____'
        resp = client.post('/admin/categories/new', data={'name': '_____', 'parent_id': '0', 'submit': 'Save'}, follow_redirects=True)
        ...
        resp = client.post('/admin/categories/new', data={'name': 'Eletrônicos', 'parent_id': '0', 'submit': 'Save'}, follow_redirects=True)
        results['valid_status'] = resp.status_code
        results['valid_flash'] = b"Category created." in resp.data
        results['valid_created'] = Category.query.filter_by(url_name='eletronicos').count()
