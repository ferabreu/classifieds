...
def detect_gpl2_only(info):
    ...
    or_later = any("or later" in c or "gpl v2 or later" in c for c in classifiers) or "or later" in license_field

    # fallback: license_field mentions "gpl" and "2"
    license_text_hint = "gpl" in license_field and "2" in license_field

    is_gpl2_only = (gpl2_classifier and not or_later) or (license_text_hint and not or_later)
    return is_gpl2_only, {"license_field": info_block.get("license"), "classifiers": info_block.get("classifiers")}
...
def main():
    ...
    out_lines = ["# Dependency license report\n", f"Generated by: scripts/check_licenses.py\n", "\n"]
    ...
    for raw_line, name, kind in rows:
        ...
        if kind == "vcs_or_local":
            out_lines.append(f"- Note: VCS or local entry; manual inspection required.\n\n")
        ...
        out_lines.append(f"- GPLv2-only heuristic: **{'YES' if gpl2_only else 'no'}**\n\n")
    ...
    out_lines.append(f"- Packages flagged as GPLv2-only (heuristic): {summary['gpl2_only']}\n")
    ...
    print("Manual next steps: review any flagged packages for 'GPL-2.0-only' vs 'GPL-2.0-or-later' and decide remediation.")
    return 0
